...
jQuery(function($){ 
 	...
 	function get_exchange_step1(id){
		...
		$.ajax({
			type: "POST",
			url: "https://flnex24.io/premium_action-exchange_stepselect.html?meth=post&yid=a8e632902874&ynd=0&lang=ru",
			dataType: 'json',
			data: param,
			error: function(res, res2, res3){
						console.log('Текст ошибки, text1: ' + res2 + ',text2:' + res3);
			...
			if(nowdata >= second){
				if(redir == 0){
					...
					if(durl.length > 0){
						...
						$.ajax({
							type: "POST",
							url: "https://flnex24.io/premium_action-refresh_status_bids.html?meth=post&yid=a8e632902874&ynd=0&lang=ru&auto_check=1",
							dataType: 'json',
							data: param,
							error: function(res, res2, res3){
										console.log('Текст ошибки, text1: ' + res2 + ',text2:' + res3);
...
jQuery(function($){
	...
	jQuery(function($){ 
		...
		$(document).on('click', '.captcha_reload', function(){
			...
			$.ajax({
				type: "POST",
				url: "https://flnex24.io/premium_action-captcha_reload.html?meth=post&yid=a8e632902874&ynd=0&lang=ru",
				dataType: 'json',
				data: param,
				error: function(res,res2,res3){
							console.log('Текст ошибки, text1: ' + res2 + ',text2:' + res3);
...
jQuery(function($){
	function get_table_exchange(id,id1,id2){
		...
		$.ajax({
			type: "POST",
			url: "https://flnex24.io/premium_action-table3_change_select.html?meth=post&yid=a8e632902874&ynd=0&lang=ru",
			dataType: 'json',
			data: param,
			error: function(res, res2, res3){
						console.log('Текст ошибки, text1: ' + res2 + ',text2:' + res3);
	...
	if($('#hexch_html').length > 0){
		$(document).on('click', '.js_exchange_link', function(){
			if(!$(this).hasClass('active')){
				...
				$.ajax({
					type: "POST",
					url: "https://flnex24.io/premium_action-exchange_widget.html?meth=post&yid=a8e632902874&ynd=0&lang=ru",
					dataType: 'json',
					data: param,
					error: function(res, res2, res3){
								console.log('Текст ошибки, text1: ' + res2 + ',text2:' + res3);
...
jQuery(function($){
	...
	function go_exchange_calc(sum, dej){
        ...
        $.ajax({
            type: "POST",
            url: "https://flnex24.io/premium_action-exchange_calculator.html?meth=post&yid=a8e632902874&ynd=0&lang=ru",
            data: param,
	        dataType: 'json',
 			error: function(res, res2, res3){
						console.log('Текст ошибки, text1: ' + res2 + ',text2:' + res3);
